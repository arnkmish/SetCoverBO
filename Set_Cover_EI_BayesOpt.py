...
def EI(x_new,best_sol_val,regr1):
	...
	Phi_1= norm.cdf(((regr1.predict(x_new)-best_sol_val-1.576) / (np.std(tree_list))))
	Phi_2=norm.pdf(((regr1.predict(x_new)-best_sol_val-1.576) / (np.std(tree_list))))
	Exp_imp= (((regr1.predict(x_new)-best_sol_val-1.576)*Phi_1)+((np.std(tree_list))*Phi_2))
	return Exp_imp

def hillclimb(x_start,best_sol_val,regr1):
	dataset = list()

	current_Sol = x_start[:]
	current_Sol_Val = EI(current_Sol,best_sol_val,regr1)
	while(True):
		childList=childListGen(current_Sol[:])
		dataset.append(current_Sol[:])
		childValues = list()
		for item in childList:
			childValues.append(EI(item,best_sol_val,regr1))
...
def stage(n,best_sol_val, regr1):
	...
	for k in range(50):	
		...
		for i in range(len(dataset)):
			valuesOfX.append(EI(listofX[i],best_sol_val, regr1))
		X = np.array(listofX)
...
def main():	
	...
	for i in range(500):
		...
		x_next = stage(n,best_sol_val,regr1)
