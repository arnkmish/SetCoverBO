...
def PI(x_new,best_sol_val,regr1):
	...
	Prob_imp= norm.cdf(((regr1.predict(x_new)-best_sol_val-1.576) / (np.std(tree_list))))
	return Prob_imp
...
def hillclimb(x_start,best_sol_val,regr1):
	...
	current_Sol = x_start[:]
	current_Sol_Val = PI(current_Sol,best_sol_val,regr1)
	while(True):
		childList=childListGen(current_Sol[:])
		dataset.append(current_Sol[:])
		childValues = list()
		for item in childList:
			childValues.append(PI(item,best_sol_val,regr1))
...
def stage(n,best_sol_val, regr1):
	...
	for k in range(500):	
		...
		for i in range(len(dataset)):
			valuesOfX.append(PI(listofX[i],best_sol_val, regr1))
		X = np.array(listofX)
